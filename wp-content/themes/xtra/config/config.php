...
if ( ! function_exists( 'codevz_theme_config' ) ) {
	 ...
	 *
	 */
	function codevz_theme_config( $key ) {

		// Theme colors
		$c1 = Codevz::option( 'site_color' );
		$c2 = Codevz::option( 'site_color_sec' );

		$c = array();

		$c['version_key'] = null; // Reset options (string)

		$c['textdomain'] = 'xtra'; // Text domain, should be lowercase

		$c['translation'] = esc_html__( 'Xtra', 'xtra' ); // Fix translation

		// Server URL of demos and plugins
		$c['server'] = 'http://xtratheme.com/';
		...
		$c['plugins'] = array(
			array(
				...
				'slug' 				=> 'codevz-plus',
				'version' 			=> '2',
				'source' 			=> $c['server'] . 'plugins/codevz-plus.zip',
				'required' 			=> true
			),
			array(
				...
				'slug' 				=> 'js_composer',
				'version' 			=> '5.5.2',
				'source' 			=> $c['server'] . 'plugins/js_composer.zip',
				'required' 			=> true
			),
			array(
				...
				'slug' 				=> 'revslider',
				'version' 			=> '5.4.8',
				'source' 			=> $c['server'] . 'plugins/revslider.zip',
				'required' 			=> true
			),
			array(
				'name' 				=> 'Woocommerce',
...
/**
 *
 * Required files of prests and templates
 *
 */
if ( file_exists( Codevz::$dir . '/config/presets/_presets.php' ) ) {
	require_once( Codevz::$dir . '/config/presets/_presets.php' );
}
if ( file_exists( Codevz::$dir . '/config/templates/_templates.php' ) ) {
	require_once( Codevz::$dir . '/config/templates/_templates.php' );
}

if ( ! function_exists( 'codevz_default_options' ) ) {
	/**
	 *
	 ...
	 *
	 */
	function codevz_default_options() {
		return array(
			'layout' => 'right',
			'primary' => 'primary',
			'secondary' => 'secondary',
			...
			array(
				...
				array(
					...
					'image_width' => '',
					'image_link' => '',
					'it_text' => 'Copyright %year%. All Rights Reserved.',
					'it_link' => '',
			...
			'_css_woo_products_add_to_cart' => 'font-size:13px;font-weight:400;background-color:#0045a0;border-radius:4px;position:absolute;bottom:100px;left:calc(50% - 65px );opacity:0.0;',
		...
		/* Theme color */a:hover, .sf-menu > .cz.current_menu > a, .sf-menu > .cz > .current_menu > a, .sf-menu > .current-menu-parent > a {color: #0045a0} 
			...
			.woocommerce .widget_price_filter .ui-slider .ui-slider-handle {background-color: #0045a0}
