...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_menus() {
			...
			if( $this->is_submenu_page() ){
				...
				$add_subpage_function( $this->parent_slug, __( 'Setup Wizard','the-simple' ), __( 'Setup Wizard','the-simple' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}else{
				add_theme_page( __( 'Setup Wizard','the-simple' ), __( 'Setup Wizard','the-simple' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => __( 'Introduction', 'the-simple' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text' => __( '...verifying','the-simple' ),
			) );
	...
	public function setup_wizard_header() {
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress Dashboard', 'the-simple' ); ?></a>
		<?php endif; ?>
		</body>
		<?php
...
//				exit;
				...
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'the-simple' ), wp_get_theme()); ?></h1>
				<p><?php _e('It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'the-simple');?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( 'Let\'s Go!', 'the-simple' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Not right now', 'the-simple' ); ?></a>
				</p>
				<?php
			} else if( get_option('envato_setup_complete', false) ){
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'the-simple' ), wp_get_theme()); ?></h1>
				<p><?php _e('It looks like you have already run the setup wizard. Below are some options: ', 'the-simple');?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php _e( 'Run Setup Wizard Again', 'the-simple' ); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Reset font style and colors', 'the-simple' ); ?>" name="save_step" />
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Cancel', 'the-simple' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'the-simple' ), wp_get_theme()); ?></h1>
				<p><?php printf( __( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.' , 'the-simple' ), wp_get_theme()); ?></p>
				<p><?php _e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'the-simple' ); ?></p>

				<h2><?php printf( __( 'Warning ! Read Me', 'the-simple' ), wp_get_theme()); ?></h2>
				...
				<ul>
					...
					<li>2. Go to WP Dashboard > The Simple > Import/Export   </li>
		...
		public function envato_setup_introduction_save(){
			...
			if(!empty($_POST['reset-font-defaults']) && $_POST['reset-font-defaults'] == 'yes'){
				...
				update_option('simple_redata', $data);

				$data1 = ReduxFrameworkInstances::get_instance('simple_redata')->options_defaults;

				update_option('simple_redata', $data1);
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php _e( 'Default Plugins', 'the-simple' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php _e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'the-simple' ); ?></p>
					...
					echo '<p><strong>'._e( 'Good news! All plugins are already installed and up to date. Please continue.','the-simple' ).'</strong></p>';
				} ?>

				<p><?php _e( 'You can add and remove plugins later on from within WordPress.', 'the-simple' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php _e( 'Continue', 'the-simple' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'the-simple' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No Slug Found','the-simple' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Activating Plugin','the-simple' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Updating Plugin','the-simple' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Installing Plugin','the-simple' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => __( 'Success','the-simple' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach($available_content as $post_type => $post_data){
				if(count($post_data)){
					...
					$content[$post_type] = array(
						...
						'description' => sprintf( __( 'This will create default %s as seen in the demo.', 'the-simple' ), $post_type_title ),
						'pending' => __( 'Pending.', 'the-simple' ),
						'installing' => __( 'Installing.', 'the-simple' ),
						'success' => __( 'Success.', 'the-simple' ),
			...
			$content['widgets'] = array(
				'title' => __( 'Widgets', 'the-simple' ),
				'description' => __( 'Insert default sidebar widgets as seen in the demo.', 'the-simple' ),
				'pending' => __( 'Pending.', 'the-simple' ),
				'installing' => __( 'Installing Default Widgets.', 'the-simple' ),
				'success' => __( 'Success.', 'the-simple' ),
			...
			$content['revslider'] = array(
				'title' => __( 'Revolution Slider', 'the-simple' ),
				'description' => __( 'Install Revolution Sliders.', 'the-simple' ),
				'pending' => __( 'Pending.', 'the-simple' ),
				'installing' => __( 'Installing Revolution Sliders... ', 'the-simple' ),
				'success' => __( 'Success.', 'the-simple' ),
			...
			$content['options'] = array(
				'title' => __( 'Theme Option', 'the-simple' ),
				'description' => __( 'Configure default settings.', 'the-simple' ),
				'pending' => __( 'Pending.', 'the-simple' ),
				'installing' => __( 'Installing Default Settings.', 'the-simple' ),
				'success' => __( 'Success.', 'the-simple' ),
			...
			$content['attachments'] = array(
				'title' => __( 'Install Images', 'the-simple' ),
				'description' => __( 'Install Img and attachments', 'the-simple' ),
				'pending' => __( 'Pending.', 'the-simple' ),
				'installing' => __( 'Installing attachments.', 'the-simple' ),
				'success' => __( 'Success.', 'the-simple' ),
				'install_callback' => array( $this,'_content_install_attachments' ),
				'checked' => 0 // dont check if already have content installed.
			);

           
		    $content['demo'] = array(
				'title' => __( 'Install Content', 'the-simple' ),
				'description' => __( 'Install Content of the Demo.', 'the-simple' ),
				'pending' => __( 'Pending.', 'the-simple' ),
				'installing' => __( 'Installing Demo.', 'the-simple' ),
				'success' => __( 'Success.', 'the-simple' ),
			...
			$content['menu'] = array(
				'title' => __( 'Menu', 'the-simple' ),
				'description' => __( 'Insert default menu as seen in the demo.', 'the-simple' ),
				'pending' => __( 'Pending.', 'the-simple' ),
				'installing' => __( 'Installing Default Menu.', 'the-simple' ),
				'success' => __( 'Success.', 'the-simple' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php _e( 'Choose one demo to be installed', 'the-simple' ); ?></h1>
			<form method="post">
				<?php if($this->is_possible_upgrade()){ ?>
					<p><?php _e('It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'the-simple'); ?></p>
				<?php }else{ ?>
					<p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'the-simple' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				...
				<table class="envato-setup-pages" cellspacing="0">
					...
					<tr>
						<td class="check"> </td>
						<th class="item"><?php _e( 'Item', 'the-simple' ); ?></th>
						<th class="description"><?php _e( 'Description', 'the-simple' ); ?></th>
						<th class="status"><?php _e( 'Status', 'the-simple' ); ?></th>
					</tr>
					</thead>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php _e( 'Continue', 'the-simple' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'the-simple' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No content Found','the-simple' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'error' => 1, 'message' => __( 'Error','the-simple' ) ) );
			}
...
//									echo "Post meta $meta_key was $meta_val \n\n";
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( !$upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && !empty($response['body']) && $response['response']['code'] == '200' ) {
					...
					@unlink( $upload['file'] );
					return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'the-simple' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'the-simple' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'the-simple' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'the-simple' ), size_format( $max_size ) ) );
				}
			}
		...
		private function _content_install_widgets() {
			...
			if($options){
				...
				$this->simple_import_sidebars_widgets($file);
				$msg = '<br />'.__("Widgets Imported !", "the-simple");
				return true;
			}else{
				$msg = '<br />'.__("Your backup or demo doesn't contain sidebar_widgets.txt file.", "the-simple");
				return false;
			}
		...
		function simple_import_sidebars_widgets($file){
	    ...
	    public function modifyXml (){
		    ...
		    $path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/' . $file.'.xml';
				...
				foreach($items as $item){
					...
					$wp_children->attachment_url = str_replace('http://thesimple.ellethemes.com/default/wp-content/uploads/', get_template_directory_uri().'/downloads/uploads/', $wp_children->attachment_url);
		...
		public function _content_install_menu(){
			...
			if($menus_data){
				...
				$this->message .= '<br />'.__("Menu Imported successfully", "the-simple");
				return true;
			}else{
				$this->message .= '<br />'.__("Your backup or demo doesn't contain sidebar_widgets.txt file.", "the-simple");
				return true;
			}
		...
		public function _content_install_revsliders(){
		   ...
		   $dir = $_POST['demo'];	
			...
			$slider_array = array('slider' => 'slider_zips/'.$dir.'/slider.zip', 'product_presentation'=>'slider_zips/product_presentation/slider.zip', 'product_presentation1'=>'slider_zips/product_presentation1/slider.zip'  );

			
			foreach ($slider_array as $sliders => $slider_url  ) {
				

					$uri = 'http://thesimple.ellethemes.com/'.$slider_url;
		...
		public function _content_install_options(){
			...
			if($options){
				update_option('simple_redata', $options);
				
				$msg = '<br />'.__("Options imported successfully", "the-simple");
				return true;
			}else{
				$msg = '<br />'.__("Your backup doesn't contain options.txt file.", "the-simple");
				return false;
			}
		}
	    ...
	    public function _content_install_demo() {
	    	...
	    	$file = 'content_1';
				...
				require_once( ABSPATH. 'wp-content/plugins/elle-framework/admin/inc/fields/codeless_import/class.wordpress-importer.php');
			...
			$path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/'. $file.'.xml';
				...
				if(is_wp_error($value)){
					$this->message .= __("Error during Import. Try Again", "the-simple");
					
					return true;
				}
				else {
					$this->message .= __("Content has been imported successfully ", "the-simple");
		...
		public function read_file($file, $dir){
			...
			if ( file_exists($file) ) {
				...
				$this->message = __("File doesn't exist", "the-simple");
			}
		...
		public function envato_setup_logo_design() {
			 global $simple_redata;
			?>
			<h1><?php _e( 'Logo &amp; Design', 'the-simple' ); ?></h1>
			<form method="post">
				<p><?php printf( __( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.', 'the-simple'), ''); ?></p>

				<table>
					<tr>
						<td>
							<div id="current-logo">
								<?php $image_url = $simple_redata['logo']['url'];
						...
						<td>
							<a href="#" class="button button-upload"><?php _e( 'Upload New Logo', 'the-simple' ); ?></a>
						</td>
					</tr>
				</table>
				...
				if( !$this->get_default_theme_style() || !$demo_styles){
					...
					<p><?php _e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'the-simple' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'the-simple' ); ?>" name="save_step" />
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'the-simple' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function envato_setup_logo_design_save() {
			...
			if ( $new_logo_id ) {
				...
				if ( $attr && ! empty( $attr[1] ) && ! empty( $attr[2] ) ) {
					$simple_redata['logo']['url'] = $attr[0];
					update_option('simple_redata', $simple_redata);
		...
		public function envato_setup_updates() {
			?>
			<h1><?php _e( 'Theme Updates', 'the-simple' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next button-primary"><?php _e( 'Continue', 'the-simple' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php _e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'the-simple' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
							<em>On the next page you will be asked to Login with your ThemeForest account and grant permissions to enable Automatic Updates. If you have any questions please <a href="<?php echo esc_url("http://ellethemes.com")?>" target="_blank">contact us</a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Login with Envato', 'the-simple' ); ?>" name="save_step" />
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'the-simple' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
					<?php } ?>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Continue', 'the-simple' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p>Item Support can be accessed from <a href="<?php echo esc_url("http://ellethemes.com/support")?>" target="_blank">http://ellethemes.co/support</a> and includes:</p>
			<ul>
			...
			<ul>
				...
				<li>Installation services & Hosting</li>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Agree and Continue', 'the-simple' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			?>
			<a href="<?php echo esc_url("https://twitter.com/share") ?>" class="twitter-share-button" data-url="http://themeforest.net/user/ellethemes/portfolio?ref=ellethemes" data-text="<?php echo esc_attr( 'I just installed the ' . wp_get_theme() .' #WordPress theme from #ThemeForest' ); ?>" data-via="EnvatoMarket" data-size="large">Tweet</a>
			<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>

			<h1><?php _e( 'Your Website is Ready!', 'the-simple' ); ?></h1>
			...
			<p>Please come back and <a href="<?php echo esc_url("http://themeforest.net/downloads") ?> " target="_blank">leave a 5-star rating</a> if you are happy with this theme. <br/>Follow <a  href="https://twitter.com/ellethemes" target="_blank">@ellethemes</a> on Twitter to see updates. Thanks! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php _e( 'Next Steps', 'the-simple' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large" href="https://twitter.com/ellethemes" target="_blank"><?php _e( 'Follow @ellethemes on Twitter', 'the-simple' ); ?></a></li>
						<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url( home_url('/') ); ?>"><?php _e( 'View your new website!', 'the-simple' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php _e( 'More Resources', 'the-simple' ); ?></h2>
					<ul>
						<li class="documentation"><a href="<?php echo esc_url("http://ellethemes.com/documentation")?>" target="_blank"><?php _e( 'Read the Theme Documentation', 'the-simple' ); ?></a></li>
						<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php _e( 'Learn how to use WordPress', 'the-simple' ); ?></a></li>
						<li class="rating"><a href="<?php echo esc_url("http://themeforest.net/downloads") ?>" target="_blank"><?php _e( 'Leave an Item Rating', 'the-simple' ); ?></a></li>
						<li class="support"><a href="<?php echo esc_url("http://ellethemes.com")?>" target="_blank"><?php _e( 'Get Help and Support', 'the-simple' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class = 'error';
					echo "<div class=\"$class\"><p>".sprintf( __( 'Something went wrong while trying to retrieve oauth token: %s','the-simple' ), $error_message ).'</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( __( 'Login for %s updates', 'the-simple' ), $this->envato_username ),
