...
function qem_event_construct ($atts) { 
    ...
    if (($display['uselistics'] && !is_singular ('event')) || ($display['useics'] && !$atts['widget']&& is_singular ('event'))) {
        $content .= '<p>'.qem_ics_button($post->ID).'</p>';
    }
...
function qem_build_event ($name,$event,$display,$custom,$atts,$register,$payment) {
    ...
    switch ( $name ) {
        ...
        case 'field14':
            ...
            if ($event['ics_label'] && (!$display['useics'] || !$display['uselistics'])) {
                $output .= qem_ics_button($post->ID);
            }
            $output .='</h4>';
        break;
    }
...
function qem_head_ic () {
    ...
    if (is_singular ('event') ) {
        $unixtime = get_post_meta($post->ID, 'event_date', true);
        $date = date_i18n("j M y", strtotime($unixtime));
...
function qem_ics_button($id) {
	return "<a class='qem_ics_button' href='".admin_url( 'admin-ajax.php?action=qem_download_ics&id='.$id )."' target='_blank'>Add To Calendar</a>";
}
...
function qem_create_post($date,$post_id,$publish,$duplicate) {
    ...
    $post = $duplicate ? get_post($post_id) : get_post($post);
