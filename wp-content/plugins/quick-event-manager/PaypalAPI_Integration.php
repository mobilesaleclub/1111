<?php
	...
	*/
	function qem_get_incontext() {
		...
		if ($mode == 'SANDBOX') { $incontext = qem_get_stored_sandbox(); }
		else { $incontext = qem_get_stored_incontext(); }
	...
	function qem_display_success($return,$return_url, $api) {
		...
		if (isset($return['PAYMENTINFO_0_TRANSACTIONID'])) $tid = $return['PAYMENTINFO_0_TRANSACTIONID'];
		else $tid = $return['TRANSACTIONID'];
	...
	*/
	...
	function qem_stripe_success($lines,$return_url, $api) {
        $payment = qem_get_stored_payment();
		...
		foreach ($lines as $line) {
			$display .= "<p><b>{$line['k']}</b> {$line['v']}</p>";
		}
		
        $display .= "<p><a href=\"{$return_url}\">{$api['confirmationanchor']}</a></p>";
        return $display;
	}

	function qem_stripe_pending($lines,$api) {
		$display = '<h2>'.$api['pendingtitle'].'</h2>
        <p>'.$api['pendingblurb'].'</p>';
		return $display;
	}

	function qem_stripe_failure($lines, $return_url, $api) {
		$display = '<h2>'.$api['failuretitle'].'</h2>
        <p>'.$api['failureblurb'].'</p>
        <p><a href='.$return_url.'>'.$api['failureanchor'].'</a></p>';
		return $display;
