 ...
 *  'before': (string)
 *  'after': (string)
 *  'options': (string|array) Used to overwrite options set in WP-Admin -> Settings -> QEMNavi
 *
 * @return void|string
 */
function wp_qemnavi( $args = array() ) {
	if ( !is_array( $args ) ) {
		$argv = func_get_args();
	...
	$options = wp_parse_args( $options, QEMNavi_Core::$options->get() );

	$instance = new QEMNavi_Call( $args );
	...
	$class_names = array(
		'pages' => apply_filters( 'wp_qemnavi_class_pages', 'pages'),
		'first' => apply_filters( 'wp_qemnavi_class_first', 'first' ),
		'previouspostslink' => apply_filters( 'wp_qemnavi_class_previouspostslink', 'previouspostslink' ),
		'extend' => apply_filters( 'wp_qemnavi_class_extend', 'extend' ),
		'smaller' => apply_filters( 'wp_qemnavi_class_smaller', 'smaller' ),
		'page' => apply_filters( 'wp_qemnavi_class_page', 'page' ),
		'current' => apply_filters( 'wp_qemnavi_class_current', 'current'),
		'larger' => apply_filters( 'wp_qemnavi_class_larger', 'larger' ),
		'nextpostslink' => apply_filters( 'wp_qemnavi_class_nextpostslink', 'nextpostslink'),
		'last' => apply_filters( 'wp_qemnavi_class_last', 'last'),
	);
		...
		case 1:
			...
			if ( !empty( $options['pages_text'] ) ) {
				$pages_text = str_replace(
					...
					__( $options['pages_text'], 'wp-qemnavi' ) );
				$out .= "<span class='{$class_names['pages']}'>$pages_text</span>";
			}

			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				// First
				$first_text = str_replace( '%TOTAL_PAGES%', number_format_i18n( $total_pages ), __( $options['first_text'], 'wp-qemnavi' ) );
			...
			if ( $end_page < $total_pages ) {
				// Last
				$out .= $instance->get_single( $total_pages, __( $options['last_text'], 'wp-qemnavi' ), array(
	...
	$out = apply_filters( 'wp_qemnavi', $out );
