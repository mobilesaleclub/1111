...
jQuery(document).ready(function () {
    ...
    jQuery('#qem_upload_stripe_image').click(function(e) {
        e.preventDefault();
        ...
        custom_uploader.on('select', function() {
            ...
            jQuery('#qem_stripe_image').val(attachment.url);
        });
...
function qem_handle_change() {
	...
	var selects = {'for':jQuery('select[name=thewmy]'),'target':jQuery('select[name=theday]'),'frequency':jQuery('select[name=thenumber]')};

	// Get the current values before swapping
	var val = {'for':selects.for.val() || false,'target': selects.target.val() || false,'frequency':selects.frequency.val() || false};
